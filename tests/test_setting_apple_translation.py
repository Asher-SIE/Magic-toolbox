...
class AppleTranslationPlatformTests(unittest.TestCase):
    ...
    @mock.patch("setting.subprocess.run", return_value=completed.__func__("26.0.1\n"))
    def test_macos_26_is_supported(self, _run):
        self.assertTrue(setting.supports_apple_translation())

    @mock.patch("setting.subprocess.run", return_value=completed.__func__("15.7.2\n"))
    def test_macos_15_is_not_supported(self, _run):
